...
const directions = [
  "img/assets/png/playerLeft.png",
  "img/assets/png/player.png",
  "img/assets/png/playerRight.png",
  "img/assets/png/playerDamaged.png"
]
...
class Ship {
  ...
  async saveScore() {
    try {
      const response = await fetch('/ranking', {
        ...
        body: JSON.stringify({ score: this.pontos }),
      });
  ...
  tirarVida(){
    ...
    if (this.vidas <= 0) {
      this.saveScore(); // Salva a pontuação no backend
      const gameOverScreen = document.getElementById("game-over-screen");
      const pontuacaoFinal = document.getElementById("pontuacao-final");
      pontuacaoFinal.textContent = `your score: ${this.pontos}`;
...
export function updateHUD(vidas, pontos) {
  ...
  for (let i = 0; i < vidas; i++) {
    ...
    img.src = "img/assets/png/life.png"; 
